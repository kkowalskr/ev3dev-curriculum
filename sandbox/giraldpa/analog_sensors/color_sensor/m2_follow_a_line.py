...
def follow_the_line(robot, white_level, black_level):
    ...
    while not robot.touch_sensor.is_pressed:
        if robot.color_sensor.reflected_light_intensity <= black_level + 10:
            while robot.color_sensor.reflected_light_intensity <= black_level + 10:
                robot.drive(500, 500)
            robot.stop()

        elif robot.color_sensor.reflected_light_intensity >= white_level - 10:
            while robot.color_sensor.reflected_light_intensity >= white_level - 10:
                robot.drive(500, -500)
            robot.stop()
